...
const CAT_LABELS={quiz:'Quiz',project:'Project',assignment:'Assignment',exam:'Exam',study:'Study',review:'Review',output:'Output',online:'Online Class',other:'Other',info:'Info',fees:'Fees',suspension:'Suspension',noclasses:'No Classes',event:'Event',fillup:'Fill Up',learning:'Learning Task'};
...
function getAdminChipClass(name){
  if(!name) return '';
  return 'chip-'+name.toLowerCase();
}
...
function userBtnClick(){
  if(currentRole==='admin') openLogoutModal();
}
...
function updateRoleUI(){
  ...
  if(isAdmin){
    ...
    const nameClass='admin-'+currentAdminName.toLowerCase();
    adminBtn.className=`role-btn admin-active ${nameClass}`;
...
function renderFeatured(){
  ...
  const feat=tasks.filter(t=>{if(t.done)return false;const s=_getStatus(t);return s.over||s.today||s.soon;})
    .sort((a,b)=>{
  ...
  if(featBadge){
    ...
    featBadge.textContent=total;
    featBadge.style.display=total>0?'':'none';
  }
...
function buildCard(t){
  ...
  const imgHTML=imgs.map((src,i)=>`<div class="card-img-thumb" onclick="openLightbox('${t.id}',${i})"><img src="${esc(src)}" alt="" loading="lazy"/></div>`).join('');
  ...
  if(currentRole==='admin'){
    expandedFooter=`<div class="card-footer-exp">
      ...
      <button class="cfb-icon copy" id="copy-e-${t.id}" onclick="event.stopPropagation();copyDesc('${t.id}','copy-e-${t.id}')" title="Copy description">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
      </button>
      <span class="cfb-spacer"></span>
      ${creatorChip}
...
function expandCard(id){
  ...
  if(activeFilter!=='all'){
    const s=_getStatus(t);
    const visible=(activeFilter==='today'&&s.today)||(activeFilter==='soon'&&s.soon)||(activeFilter==='overdue'&&s.over)||(activeFilter==='done'&&t.done);
    if(!visible){
      activeFilter='all';
      _getFilterBtns().forEach(b=>b.classList.remove('active'));
...
function _lbUpdate(){
  const t=tasks.find(x=>x.id===_lbTaskId);if(!t)return;
  const imgs=t.images||[];
...
function confirmPhotoDelete(){
  ...
  persist();
  if(t.images.length===0){closeLightbox();renderTasks();return;}
  _lbIdx=Math.min(_lbIdx,t.images.length-1);
  _lbUpdate();
  renderTasks();
}
...
function saveTask(){
  ...
  if(editId){
    ...
    tasks.push({
      ...
      created:Date.now(),images:[],done:false,_expanded:false,
...
function saveNote(){
  ...
  notes.push({
    ...
    text,author:currentAdminName,createdAt:Date.now(),
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    closeModal();closeLightbox();closeLoginModal();closeDelModal();closeNoteModal();closeLogoutModal();closePhotoDelModal();
  }
...
// ─────────────────────────────────────────────
//  MOBILE KEYBOARD — keep modals above keyboard
// ─────────────────────────────────────────────
(function(){
  ...
  function applyViewport(){
    ...
    if(!vv) return;
    const vvHeight=vv.height, vvOffsetTop=vv.offsetTop, vvOffsetLeft=vv.offsetLeft;
    SHEET_OVERLAYS.forEach(id=>{
      const el=document.getElementById(id);
      if(!el) return;
      el.style.top=vvOffsetTop+'px'; el.style.left=vvOffsetLeft+'px';
      el.style.right='0px'; el.style.height=vvHeight+'px'; el.style.bottom='auto';
      const modal=el.querySelector('.modal');
      if(modal) modal.style.maxHeight=Math.floor(vvHeight*0.9)+'px';
    });
  }
